#define init
global.sprButton = sprite_add("Sprites/Main/Sloppy Fingers.png", 1, 12, 16)
global.sprIcon = sprite_add("Sprites/Icons/Sloppy Fingers Icon.png", 1, 8, 8)
...
with(Player){
	with(instances_matching(instances_matching_ne([Slash, GuitarSlash, BloodSlash, EnergySlash, Shank, EnergyShank, SawBurst, EnergyHammerSlash], "Sloppy_Fingers", 1), "team", team)){
		image_angle += (irandom(90)-45)*(other.accuracy)*(skill_get(mut_eagle_eyes) ? 0.5 : 1);
		Sloppy_Fingers=1;
	}
	with(instances_matching(instances_matching_ne(projectile, "Sloppy_Fingers", 1), "team", team)){
		var r = (irandom(20)-10)*(skill_get(mut_eagle_eyes) ? 0.1 : 1);

#define init
global.sprButton = sprite_add("Sprites/Main/Rocket Casings.png", 1, 12, 16)
global.sprIcon = sprite_add("Sprites/Icons/Rocket Casings Icon.png", 1, 8, 8)
...
with(Player){
	...
	with(instances_matching(instances_matching_ne(CustomProjectile,"rocketcasings",true),"team",team)){
		if("name" in self && is_string(name) && string_count("bullet", string_lower(name)) > 0 || "hitid" in self && is_array(hitid) && hitid[1] == "SHOCKROUND"){
			speed = speed + 3;
			force = force * 2;
			rocketcasings = true;
			if(fork()){

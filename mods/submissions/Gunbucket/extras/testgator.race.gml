...
#define init
	...
	/// Define Sprites : sprite_add("path/to/sprite/starting/from/mod/location.png", frames, x-offset, y-offset) \\\
	 // A-Skin:
	global.spr_idle[0] = sprite_add("sprites/gatorrace/sprMutant1Idle.png",	4, 12, 12);
	global.spr_walk[0] = sprite_add("sprites/gatorrace/sprMutant1Walk.png",	6, 12, 12);
	global.spr_hurt[0] = sprite_add("sprites/gatorrace/sprMutant1Hurt.png",	3, 12, 12);
	global.spr_dead[0] = sprite_add("sprites/gatorrace/sprMutant1Dead.png",	6, 12, 12);
	global.spr_sit1[0] = sprite_add("sprites/gatorrace/sprMutant1GoSit.png",	3, 12, 12);
	global.spr_sit2[0] = sprite_add("sprites/gatorrace/sprMutant1Sit.png",	1, 12, 12);

	 // B-Skin:
	global.spr_idle[1] = sprite_add("sprites/gatorrace/sprMutant1BIdle.png",	4, 12, 12);
	global.spr_walk[1] = sprite_add("sprites/gatorrace/sprMutant1BWalk.png",	6, 12, 12);
	global.spr_hurt[1] = sprite_add("sprites/gatorrace/sprMutant1BHurt.png",	3, 12, 12);
	global.spr_dead[1] = sprite_add("sprites/gatorrace/sprMutant1BDead.png",	6, 12, 12);
	global.spr_sit1[1] = sprite_add("sprites/gatorrace/sprMutant1BGoSit.png",	3, 12, 12);
	global.spr_sit2[1] = sprite_add("sprites/gatorrace/sprMutant1BSit.png",	1, 12, 12);
	
	 // Character Selection / Loading Screen:
	global.spr_slct = sprite_add("sprites/gatorrace/sprCharSelect.png",	1,				0,  0);
	global.spr_port = sprite_add("sprites/gatorrace/sprBigPortrait.png",	race_skins(),	40, 243);
	global.spr_skin = sprite_add("sprites/gatorrace/sprLoadoutSkin.png",	race_skins(),	16, 16);
	global.spr_icon = sprite_add("sprites/gatorrace/sprMapIcon.png",		race_skins(),	10, 10);

	 // Ultras:
	global.spr_ult_slct = sprite_add("sprites/gatorrace/sprEGSkillIcon.png",	ultra_count("fishexample"), 12, 16);
	global.spr_ult_icon[1] = sprite_add("sprites/gatorrace/sprEGIconHUDA.png", 1, 8, 9);
	global.spr_ult_icon[2] = sprite_add("sprites/gatorrace/sprEGIconHUDB.png", 1, 8, 9);
...
#define create
	...
	/// PASSIVE : More Ammo \\\
	typ_ammo[1] = 32;	// +8 Bullets
	typ_ammo[2] = 8;	// +2 Shells
	typ_ammo[3] = 7;	// +2 Bolts
	typ_ammo[4] = 6;	// +2 Explosives
	typ_ammo[5] = 10;	// +3 Energy
...
#define step
	...
	if(canspec){
		...
		if(button_pressed(index, "spec") && speed <= 10) direction = gunangle;
	...
	if(rollTime > 0){
		...
		if(skill_get(5)){
			...
			instance_create(x,y,CaveSparkle);	// Water Particles
		}

		/// Roll (No Butt) \\\
		else{
			canwalk = 1;				// Can't Use Movement Keys
		...
		with instance_create(x, y, GreenExplosion)
		{
	     ...
	     // Touch Throne:
		if(place_meeting(x + hspeed, y + vspeed, Nothing)){
			move_bounce_solid(true);
			rollTime *= skill_get(5);
...
#define race_name
 ...
 // Description:
#define race_text
	return "BROKEN TEST CHARACTER#@rREMOVE BEFORE RELEASE";

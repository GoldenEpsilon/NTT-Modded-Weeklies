...
if array_length(instances_matching(CustomObject,"name","mod_ui_options_button"))<1{
    with instance_create(game_width-4-(20*button_position_from_right),7,CustomObject){
        ...
        while instance_exists(self){
            ...
            for(var i=0;i<maxp;i++){
                if instance_exists(self) and (Menu.menu_hover == 1 or point_in_rectangle(mouse_x[i],mouse_y[i],view_xview[i]+x,view_yview[i]+y,view_xview[i]+x+sprite_get_width(sprite_index),view_yview[i]+y+sprite_get_height(sprite_index))){
                    //trace(choose(""," ","  "),"hover")
